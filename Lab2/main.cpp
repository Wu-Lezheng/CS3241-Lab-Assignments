...
// COMMENTS TO GRADER: Explanation of each step is in the comments.
//
// ============================================================
...
void DrawCarBody() {
    ...
    glPushMatrix();
        // Translate the cube such that its back surface coincides with the xy-plane.
        glTranslated(0, 0, CAR_HEIGHT / 4);
        // Scale the cube to the size of the car but half the height.
    ...
    // Draw the top cube.
    glPushMatrix();
        // Translate the cube such that its back surface coincides with the front surface of the bottom cube.
        glTranslated(0, 0, CAR_HEIGHT * 3 / 4);
        // Scale the cube to the size of the car but half the height.
...
void DrawTyre() {
    ...
    // Define radii of the torus.
    double outerTyreRadius = CAR_HEIGHT / 4;
    double innerTyreRadius = outerTyreRadius / 2;

    // Define translation in x and y directions.
    double xTranslation = CAR_LENGTH / 4 + outerTyreRadius;
    double yTranslation = CAR_WIDTH / 2;

    // Create tyres at different positions.
    for (int i = 0; i < 4; i++) {
        ...
        glPushMatrix();
            ...
            // Translate the torus to its position around the car.
            glTranslated(x, y, outerTyreRadius);
            
            // Rotate the torus 90 degrees around the x-axis.
            glRotated(90, 1.0, 0, 0);
            // Draw the torus.
            glutSolidTorus(innerTyreRadius, outerTyreRadius, 30, 30);
        glPopMatrix();
    }
...
void DrawAllCars( void )
{
    for ( int i = 0; i < NUM_CARS; i++ )
    {
        //****************************
        // WRITE YOUR CODE HERE.
        CarType targetCar = car[i];
        glPushMatrix();
            // Fix the car to its track.
            glRotated(targetCar.rotAngle, targetCar.xzAxis[0], 0, targetCar.xzAxis[1]);
            // Rotate the car to its angular position.
            glRotated(targetCar.angularPos, 0, 1, 0);
            // Translate the car to the surfaceof the planet.
            glTranslated(0, 0, PLANET_RADIUS);
            // draw the car
            DrawOneCar(targetCar.bodyColor);
        glPopMatrix();
        //****************************
    }
}
...
void MyDisplay( void )
{
    ...
    //***********************************************************************
    const double DEGREE_TO_RADIANT = PI / 180;
    double eyeX = eyeDistance * cos(eyeLatitude * DEGREE_TO_RADIANT) * sin(eyeLongitude * DEGREE_TO_RADIANT);
    double eyeY = eyeDistance * sin(eyeLatitude * DEGREE_TO_RADIANT);
    double eyeZ = eyeDistance * cos(eyeLatitude * DEGREE_TO_RADIANT) * cos(eyeLongitude * DEGREE_TO_RADIANT);
