...
// COMMENTS TO GRADER: There is a parser to parse the Wavefront.obj file. The 3D model is loaded in GLInit().
//
// ============================================================
...
const char titaniumTexFile[] = "images/titanium.jpg";
...
void MakeReflectionImage( void )
{
    ...
    float virtualEyePos[3] = { eyePos[0], eyePos[1], 2 * TABLETOP_Z - eyePos[2] };
...
// This texture object is for storing the reflection image read from the color buffer.
    ...
    // This texture object is for the titanium texture map.
    glGenTextures(1, &rocketTexObj);
    glBindTexture(GL_TEXTURE_2D, rocketTexObj);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);

    if (ReadImageFile(titaniumTexFile, &imageData,
        &imageWidth, &imageHeight, &numComponents) == 0) exit(1);
    if (numComponents != 3)
    {
        fprintf(stderr, "Error: Texture image is not in RGB format.\n");
...
// Draw the rocket and place it above the table.
void DrawRocket( void ) {
    ...
    glPushMatrix();
    glTranslated((TABLETOP_X2 - TABLETOP_X1) / 4, -(TABLETOP_Y2 - TABLETOP_Y1) / 4,0);
    glTranslated(0, 0, 1.8 * TABLETOP_Z);
    glRotated(90, 1, 0, 0);
    model.draw();
    glPopMatrix();
