...
void DrawDisc( const discType *d )
{
    ...
    if ( firstTime )
    {
        ...
        {
            double angle = 2 * PI * i / NUM_OF_SIDES;
            double x = cos(angle);
            double y = sin(angle);
            unitDiscVertex[i][0] = x;
            unitDiscVertex[i][1] = y;
        }
    ...
    //*** WRITE YOUR CODE HERE ***
    //****************************
    GLdouble xOfOrigin = d->pos[0];
    GLdouble yOfOrigin = d->pos[1];
    GLdouble radius = d->radius;
    glColor3ub(d->color[0], d->color[1], d->color[2]);
    glBegin(GL_TRIANGLE_FAN);
        glVertex2d(xOfOrigin, yOfOrigin);
        for (int j = 0; j <= NUM_OF_SIDES; j++) 
        {
            GLdouble x = xOfOrigin + radius * unitDiscVertex[j][0];
            GLdouble y = yOfOrigin + radius * unitDiscVertex[j][1];
            glVertex2d(x, y);
        }
    glEnd();
}
...
void MyMouse( int btn, int state, int x, int y )
{
    if ( btn == GLUT_LEFT_BUTTON && state == GLUT_DOWN )
    {
        ...
        else
        {
            ...
            double randomRadius = MIN_RADIUS + rand() % radiusRange;
            disc[numDiscs].radius = randomRadius;

            // generate random RGB
            for (int i = 0; i < 3; i++)
            {
                disc[numDiscs].color[i] = (unsigned char)(rand() % 256);
            }

            // generate random speed
            int xSpeedRange = MAX_X_SPEED - MIN_X_SPEED;
            int ySpeedRange = MAX_Y_SPEED - MIN_Y_SPEED;
            double randomXSpeed = MIN_X_SPEED + rand() % xSpeedRange;
            double randomYSpeed = MIN_Y_SPEED + rand() % ySpeedRange;
            int a = rand() % 2 + 1;
            int b = rand() % 2 + 1;
            if (a % 2) {
            ...
            disc[numDiscs].speed[0] = randomXSpeed;
            disc[numDiscs].speed[1] = randomYSpeed;
...
void UpdateAllDiscPos( void )
{
    for ( int i = 0; i < numDiscs; i++ )
    {
        ...
        //*** WRITE YOUR CODE HERE ***
        //****************************
        double x = disc[i].pos[0];
        double y = disc[i].pos[1];
        double xSpeed = disc[i].speed[0];
        double ySpeed = disc[i].speed[1];
        double radius = disc[i].radius;

        x += xSpeed;
        y += ySpeed;

        double leftPosition = x - radius;
        double rightPosition = x + radius;
        double bottomPosition = y - radius;
        double topPosition = y + radius;
        
        // get x value
        if (leftPosition < 0)
        {
            disc[i].pos[0] = radius;
            disc[i].speed[0] = - xSpeed;
        }
        else if (rightPosition > winWidth)
        {
            disc[i].pos[0] = winWidth - radius;
            disc[i].speed[0] = - xSpeed;
        }
        else
        {
        ...
        {
            disc[i].pos[1] = radius;
            disc[i].speed[1] = - ySpeed;
        }
        else if (topPosition > winHeight)
        {
            disc[i].pos[1] = winHeight - radius;
            disc[i].speed[1] = - ySpeed;
        }
        else
        {
