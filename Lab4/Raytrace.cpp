...
// Compute for shadow if hasShadow is true.
    ...
    //***********************************************
    Vector3d nearestPoint = nearestHitRec.p;
    for (const auto& light : scene.ptLights) {
        Vector3d L = light.position - nearestPoint;
        Vector3d unitL = L.unitVector();
        Color phongLightingResult = computePhongLighting(unitL, N, V, nearestHitRec.material, light);

        if (hasShadow) {
            Ray shadowRay = Ray(nearestPoint, unitL);
            Color k_s = Color(1.0f, 1.0f, 1.0f);

            for (const auto& surface : scene.surfaces) {

                if (surface->shadowHit(shadowRay, DEFAULT_TMIN, L.length())) {
                    k_s.setR(0.0f);
                    k_s.setG(0.0f);
                    k_s.setB(0.0f);
                    break;
            ...
            phongLightingResult *= k_s;

        }

        result += phongLightingResult;
    }
...
// Add to result the reflection of the scene.
    ...
    if (reflectLevels == 0) {
        ...
        Ray reflection = Ray(nearestPoint, R);
        return result += nearestHitRec.material.k_rg * TraceRay(reflection, scene, reflectLevels - 1, hasShadow);
    }
